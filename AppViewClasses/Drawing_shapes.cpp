...
void Leader_north::drawself(CGContextRef ctx, const VisualViewObject * const obj_ptr) const
{
	...
	float len = (sz.width > sz.height) ? sz.width : sz.height;
	CGContextMoveToPoint(ctx, start.x, start.y);
...
void Top_Semicircle_shape::drawself(CGContextRef ctx, const VisualViewObject * const obj_ptr) const
{
	...
	CGContextAddArc(ctx, CGRectGetMidX(rect), CGRectGetMinY(rect),  radius, 0, pi, 0);
...
void draw_clover(CGContextRef ctx, CGRect rect)
{
	...
	CGContextAddArc(ctx,  radius,      0,  radius, 6*pi/4, 2*pi/4, 0);
	CGContextAddArc(ctx,      0,  radius,  radius, 0*pi/4, 4*pi/4, 0);
	CGContextAddArc(ctx, -radius,      0,  radius, 2*pi/4, 6*pi/4, 0);
	CGContextAddArc(ctx,      0, -radius,  radius, 4*pi/4, 0*pi/4, 0);
	CGContextClosePath(ctx);
	CGContextRestoreGState(ctx);
}
